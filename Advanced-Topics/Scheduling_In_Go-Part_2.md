...
在插图8中，由`G1`产生的系统调用已经完成。此时，`G1`可以重新被移动到LRQ中，并且再次由`P`服务。`M1`则放置在旁边备用，以便再次放生这样的（基于文件IO的同步系统调用等等）情况发生。

# 工作窃取

Go调度器的另一方面就是，它也是一个工作-窃取的调度器。这样有助于在一些区域内保持有效的调度。首先，你希望的最终结果是**M**进入等待状态，因为一旦**M**进入等待状态，操作系统会将**M**从核心中切换出去。这意味着**P**无法完成任何工作，即使**G**处于可运行的状态，直到核心将**M**切换回来。窃取工作也有助于平衡所有的**P**上的**Gs**。从而更好地分配，并更加有效的完成工作。
...
在插图9中，我们有一个多线程的Go程序，其中两个`P`服务了4个`Gs`，每个服务的GRQ中有一个`G`。如果其中的一个`P`中的`Gs`服务（调度）地很快，会发生什么？
...
所以基于清单2中的这些规则，`P1`需要检查`P2`LRQ中**Gs**，并拿走已发现的**Gs**的一半。

#### 插图12：

![](https://www.ardanlabs.com/images/goinggo/94_figure12.png)

在插图12中，`P2`完成了它的工作，并且需要偷一些**Gs**。首先，它尝试从`P1`的LRQ中寻找可用的**G**，但是并没有发现。下一步，它将会从GRQ中寻找可用的**G**。这时候它发现了`G9`。
