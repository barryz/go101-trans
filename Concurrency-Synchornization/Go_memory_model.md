...
在接下来的描述中，如果我们说事件`A`将保证在事件`B`之前发生，那么它就意味着这两个事件中涉及的任何Goroutine都将观察到，源代码中的事件`A`之前出现的任何语句都将在源代码中事件`B`之后出现的任何语句之前执行。对于其他不相干的Goroutines，它们观察到的顺序可能和刚才我们描述的顺序不同。
